...
public abstract class ScheduledJob extends TimerTask{
	...
	static Logger logger = LoggerFactory.getLogger(ScheduledJob.class);
	...
	public void run() {
		if (jobStatus.get() <= 0) {
			jobStatus.set(1);			
			try {
				logger.info(name, " is started at "+new Date().toString(), true);
				doTheJob();
				logger.info(name, " is finished at "+new Date().toString(), true);
			} catch (Throwable e) {
				logger.error(name, " is crashed at "+new Date().toString(), true);
			}
			jobStatus.set(0);
		} else {
